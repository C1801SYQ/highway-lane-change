...
def add_image_centered(doc, image_path, caption="", width_inches=5.0):
    ...
    if os.path.exists(full_path):
        ...
        if caption:
            ...
            set_paragraph_spacing(cap_p, line_spacing=1.5, space_before=0, space_after=6)
...
def generate_docx():
    ...
    while i < len(lines):
        ...
        # Handle markdown image references: ![图1：caption](path)
        img_match = re.match(r'!\[(.*?)\]\((figures/.*?\.png)\)', line.strip())
        if img_match:
            caption = img_match.group(1).strip()
            img_ref = img_match.group(2).strip()
            img_filename = os.path.basename(img_ref)
            if img_filename in FIGURE_MAP.values() or os.path.exists(os.path.join(FIGURES_DIR, img_filename)):
                add_image_centered(doc, img_filename, caption=caption, width_inches=5.0)
                # Skip the italic caption line that follows
                if i + 1 < len(lines) and re.match(r'\*图\d+：', lines[i + 1].strip()):
                    i += 1
            i += 1
            continue

        # Handle old style: 【插图：figures/xxx.png】
        img_match_old = re.match(r'【插图：(.+?)】', line.strip())
        if img_match_old:
            img_ref = img_match_old.group(1).strip()
